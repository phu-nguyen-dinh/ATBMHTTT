...
CREATE OR REPLACE PROCEDURE grant_privilege_proc(
    ...
    v_sql VARCHAR2(4000);
BEGIN
    -- Nếu không có đối tượng, cấp quyền hệ thống
    IF p_object_name IS NULL THEN
        v_sql := 'GRANT ' || p_privilege || ' TO C##' || p_grantee;
    ELSE
        -- Nếu có đối tượng, cấp quyền trên đối tượng
        v_sql := 'GRANT ' || p_privilege || ' ON ' || p_object_name || ' TO C##' || p_grantee;
    END IF;
...
CREATE OR REPLACE PROCEDURE revoke_privilege_proc(
    ...
    v_sql VARCHAR2(4000);
BEGIN
    -- Nếu không có đối tượng, thu hồi quyền hệ thống
    IF p_object_name IS NULL THEN
        v_sql := 'REVOKE ' || p_privilege || ' FROM C##' || p_grantee;
    ELSE
        -- Nếu có đối tượng, thu hồi quyền trên đối tượng
        v_sql := 'REVOKE ' || p_privilege || ' ON ' || p_object_name || ' FROM C##' || p_grantee;
    END IF;
